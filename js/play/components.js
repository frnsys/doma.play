...
const EquityPurchase = View(({scene, city, tenant, p_dividend, shares}) => `
  <div>
    ...
    <div class="scene--body">
      ...
      <br /><br />
      In addition, you'll receive some direct cash back every month – about 1 cent for every dollar invested. This cashback corresponds to your share of the rental income generated by every Doma property, which are rented at a fair price and with many benefits for their tenants.
      <br /><br />
      You have $${Math.round(tenant.savings).toLocaleString()} in savings, how many equity tokens do you want to buy?</p>
      <div class="bar-annotated--labels">
        <div class="bar--label bar--label-left">
          <div>💵0</div>
        </div>
...
const EquityResultsExplained = View(({scene, state, player, results, shares}) => `
  <div class="equity-results">
    <div class="scene--stage">
      <img src="/static/scenes/${scene.image}">
    </div>
    <div class="scene--body">
      ...
      <p class="scene--desc">
        ${player.tenant.shares > 0 ? `
          Now that you have ${Math.round(shares)} equity tokens, you will receive $${Math.round(Math.round(shares)/100)} cash back every month as your share of the rental income generated by the new Doma properties. It isn't much, but it pays better than the savings accounts offered by banks!
          <br /><br />
